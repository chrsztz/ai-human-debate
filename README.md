...
两个声部不是两件乐器，是**同一条「活体度」轴上的两个位置**：

```
位置 = 固定基线（声部身份，人在机器端 / AI 在活体端）
     + 长时漂移（/debate/{spk}/vitality，整场辩论的走势）
     + 瞬时偏移（当前这一句，/debate/{spk}/seg）
```

### vitality —— 作品的头号参数

一个 0~1 的数，就是这个声部现在站在「机器 ↔ 活体」轴的哪里。Max 那边接
`[r human.vitality]` 去顶 vocality 的阈值即可（已打包成 `[目标, 3000ms]` 喂 `[line]`，
一轮一轮慢慢挪，不会跳）。

**第一版用「该说话人 embodiment 的移动平均」，实测几乎不动**：AI 均值 0.5、人 0.55，
差 0.05，听不出来。原因是结构性的 —— LLM 输出的低方差正是 post-training 的产物，
它被训练成语域稳定，再跑一百轮也一样。

改成测**离散度**：人的读数铺得开（sd ≈ 0.18）、AI 挤在中间（sd ≈ 0.06）。
这个差异大两三倍，而且**方差本身就是「活体感」的字面定义** ——
参数一直在动的声音听起来是活的，坐着不动的听起来是死的，不需要再转译一次。
...
| 项 | 性质 | 调它的参数 |
|---|---|---|
| 证据 | **观察**，方向完全由数据决定 | `DRIFT_DISPERSION_WEIGHT` `DRIFT_GAIN` |
| 置信度 | 硬、单调、保证发生，但只**放大**观察，不替代 | `DRIFT_CONFIDENCE_K` |
| 轮数斜坡 | **断言**，跟谁说了什么无关 | `DRIFT_HARD_WEIGHT`（默认 0.25） |

第一轮两个声部几乎重合（证据不足），到第十二轮拉到 0.58 的间距 —— **要的 progression 有了，
而且主要是因为证据累积，不是因为时钟在走**。如果哪一场人比 AI 更单调、更去身，
两个声部会往反方向走，那是真实发现不是 bug。

纯轮数那一项是彻头彻尾的断言，所以单独拎出来：**「统计」页会显示效果里有多少来自观察、
多少来自断言**，展览前按耳朵调 `DRIFT_HARD_WEIGHT` 时你随时知道自己用了多少。
